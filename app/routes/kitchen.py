...
def index():
    ...
    orders = query_all(
        ...
        WHERE orders.company_id = ? AND orders.status IN ('Recebido', 'Preparando', 'Pronto')
        GROUP BY orders.id
        ORDER BY orders.created_at ASC
        """,
        (cid,),
    )
    columns = [("Novos", "Recebido"), ("Em preparo", "Preparando"), ("Prontos", "Pronto")]
    orders_by_status = {status: [] for _, status in columns}
    for order in orders:
        orders_by_status.setdefault(order["status"], []).append(order)
    return render_template("kitchen.html", orders=orders, columns=columns, orders_by_status=orders_by_status)
